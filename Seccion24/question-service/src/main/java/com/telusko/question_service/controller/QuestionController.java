...
public class QuestionController {
    ...
    public ResponseEntity<List<QuestionWraper>> getQuestionsFromId(@RequestBody List<Integer> questionsId) {
        System.out.println(environment.getProperty("local.server.port")); // Print in which port is running to verify load balancing
        return questionService.getQuestionsFromIds(questionsId);
    }

    @PostMapping("getScore")
    public ResponseEntity<Integer> getScore(@RequestBody List<Response> responses) {
        return questionService.setScore(responses);
    }
}
